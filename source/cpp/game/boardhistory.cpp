      ...
      isResignation(other.isResignation)
{
  ...
  std::copy(other.wasEverOccupiedOrPlayed, other.wasEverOccupiedOrPlayed + Board::MAX_ARR_SIZE,
            wasEverOccupiedOrPlayed);
}
...
void BoardHistory::clear(const Board& board, Player pla, const Rules& r)
{
  ...
  for (int i = 0; i < NUM_RECENT_BOARDS; i++) recentBoards[i] = board;
  currentRecentBoardIdx = 0;
...
void BoardHistory::setInitialTurnNumber(int n) { initialTurnNumber = n; }
...
void BoardHistory::printDebugInfo(ostream& out, const Board& board) const
{
  ...
  out << "Game result " << isGameFinished << " " << PlayerIO::playerToString(winner) << " " << 0 << " " << 0 << " "
      << isNoResult << " " << isResignation << endl;
  out << "Last moves ";
  for (int i = 0; i < moveHistory.size(); i++) out << Location::toString(moveHistory[i].loc, board) << " ";
  out << endl;
}

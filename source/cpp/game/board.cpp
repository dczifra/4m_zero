...
const int BitBoard::index64[64] = {0,  47, 1,  56, 48, 27, 2,  60, 57, 49, 41, 37, 28, 16, 3,  61,
...
BitBoard::BitBoard(const BitBoard& other) { memcpy(t, other.t, sizeof(uint64_t) * 2); }
...
// LOCATION--------------------------------------------------------------------------------
Loc Location::getLoc(int x, int y, int x_size) { return (x + 1) + (y + 1) * (x_size + 1); }
int Location::getX(Loc loc, int x_size) { return (loc % (x_size + 1)) - 1; }
int Location::getY(Loc loc, int x_size) { return (loc / (x_size + 1)) - 1; }
...
Board::Board() { init(RULE_M, 4); }

Board::Board(int /*x*/, int /*y*/) { init(RULE_M, 4); }
...
void Board::init(int xS, int yS)
{
  ...
  for (int i = 0; i < MAX_ARR_SIZE; i++) colors[i] = C_WALL;
...
void Board::initBoardStruct()
{
  ...
  for (int i = 0; i < 4; i++) ZOBRIST_PLAYER_HASH[i] = nextHash();
...
bool Board::isOnBoard(Loc loc) const { return loc >= 0 && loc < MAX_ARR_SIZE && colors[loc] != C_WALL; }
...
bool Board::isFull() const { return (num_stones == (x_size * y_size)); }
...
string Location::toString(Loc loc, const Board& b) { return toString(loc, b.x_size, b.y_size); }

string Location::toStringMach(Loc loc, const Board& b) { return toStringMach(loc, b.x_size); }
...
Loc Location::ofString(const string& str, const Board& b) { return ofString(str, b.x_size, b.y_size); }
...
Board Board::parseBoard(int xSize, int ySize, const string& s, char lineDelimiter)
{
  ...
  for (int y = 0; y < ySize; y++) {
    ...
    while (firstNonDigitIdx < line.length() && Global::isDigit(line[firstNonDigitIdx])) firstNonDigitIdx++;

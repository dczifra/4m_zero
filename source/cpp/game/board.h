...
struct BitBoard
{
  ...
  inline void null() { this->t[0] = this->t[1] = 0; }
  ...
  inline int count() const { return __builtin_popcountll(t[0]) + __builtin_popcountll(t[1]); }
...
struct State
{
  ...
  BitBoard legal;
  BitBoard forks;
  BitBoard candidateMoves;  // TRICKY: a move is candidate only if it is part of at least two alive winning sets, or
                            // defending a win threat
  int player;               // 0 == O, 1 == X
};
...
struct Board
{
  ...
  short adj_offsets[8];  // Indices 0-3: Offsets to add for adjacent points. Indices 4-7: Offsets for diagonal points. 2
                         // and 3 are +x and +y.

...
bool Rules::operator==(const Rules& other) const
{
  return koRule == other.koRule && scoringRule == other.scoringRule && taxRule == other.taxRule &&
         ...
         whiteHandicapBonusRule == other.whiteHandicapBonusRule && komi == other.komi;
}
...
bool Rules::komiIsIntOrHalfInt(float komi) { return std::isfinite(komi) && komi * 2 == (int)(komi * 2); }

set<string> Rules::koRuleStrings() { return {"SIMPLE", "POSITIONAL", "SITUATIONAL", "SPIGHT"}; }
set<string> Rules::scoringRuleStrings() { return {"AREA", "TERRITORY"}; }
set<string> Rules::taxRuleStrings() { return {"NONE", "SEKI", "ALL"}; }
set<string> Rules::whiteHandicapBonusRuleStrings() { return {"0", "N", "N-1"}; }
...
json Rules::toJson() const { return toJsonHelper(false, false); }

json Rules::toJsonNoKomi() const { return toJsonHelper(true, false); }

json Rules::toJsonNoKomiMaybeOmitStuff() const { return toJsonHelper(true, true); }

string Rules::toJsonString() const { return toJsonHelper(false, false).dump(); }

string Rules::toJsonStringNoKomi() const { return toJsonHelper(true, false).dump(); }

string Rules::toJsonStringNoKomiMaybeOmitStuff() const { return toJsonHelper(true, true).dump(); }
...
static Rules parseRulesHelper(const string& sOrig, bool allowKomi)
{
  ...
  if (lowercased == "japanese" || lowercased == "korean") {
    ...
    rules.komi = 7.5;
  } else if (lowercased == "chineseogs" || lowercased == "chinese_ogs" || lowercased == "chinese-ogs" ||
    ...
    rules.whiteHandicapBonusRule = Rules::WHB_N;
    rules.komi = 7.5;
  } else if (lowercased == "ancientarea" || lowercased == "ancient-area" || lowercased == "ancient_area" ||
    ...
    rules.whiteHandicapBonusRule = Rules::WHB_ZERO;
    rules.komi = 7.5;
  } else if (lowercased == "ancientterritory" || lowercased == "ancient-territory" ||
    ...
    rules.whiteHandicapBonusRule = Rules::WHB_ZERO;
    rules.komi = 6.5;
  } else if (lowercased == "agabutton" || lowercased == "aga-button" || lowercased == "aga_button" ||
    ...
    rules.whiteHandicapBonusRule = Rules::WHB_N_MINUS_ONE;
    rules.komi = 7.5;
  } else if (lowercased == "nz" || lowercased == "newzealand" || lowercased == "new zealand" ||
    ...
    rules.whiteHandicapBonusRule = Rules::WHB_ZERO;
    rules.komi = 7.5;
  } else if (lowercased == "tromp-taylor" || lowercased == "tromp_taylor" || lowercased == "tromp taylor" ||
    ...
    while (true) {
      ...
      if (startsWithAndStrip(s, "komi")) {
        if (!allowKomi)
          throw IOError("Could not parse rules: " + sOrig);
        int endIdx = 0;
        while (endIdx < s.length() && !Global::isAlpha(s[endIdx] && !Global::isWhitespace(s[endIdx]))) endIdx++;
        float komi;
...
Rules Rules::parseRules(const string& sOrig) { return parseRulesHelper(sOrig, true); }

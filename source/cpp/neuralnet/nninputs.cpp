...
int NNPos::xyToPos(int x, int y, int nnXLen) { return y * nnXLen + x; }
...
int NNPos::getPolicySize(int nnXLen, int nnYLen) { return nnXLen * nnYLen + 1; }
...
NNOutput::NNOutput(const vector<shared_ptr<NNOutput>>& others)
{
  ...
  // Just give up if they don't all match in move legality
  {
    ...
    if (mismatch) {
      ...
      for (int pos = 0; pos < NNPos::MAX_NN_POLICY_SIZE; pos++) policyProbs[pos] /= floatLen;
    }
  }
}
...
static void copyWithSymmetry(const float* src, float* dst, int nSize, int hSize, int wSize, int cSize, bool useNHWC,
                             int symmetry, bool reverse)
{
...
void SymmetryHelpers::copyInputsWithSymmetry(const float* src, float* dst, int nSize, int hSize, int wSize, int cSize,
                                             bool useNHWC, int symmetry)
{
  copyWithSymmetry(src, dst, nSize, hSize, wSize, cSize, useNHWC, symmetry, false);
}

void SymmetryHelpers::copyOutputsWithSymmetry(const float* src, float* dst, int nSize, int hSize, int wSize,
                                              int symmetry)
{
...
Hash128 NNInputs::getHash(const Board& board, const BoardHistory& hist, Player nextPlayer,
...
void NNInputs::fillRowV3(const Board& board, const BoardHistory& hist, Player nextPlayer,
                         const MiscNNInputParams& /*nnInputParams*/, int nnXLen, int nnYLen, bool useNHWC,
...
void NNInputs::fillRowV4(const Board& board, const BoardHistory& hist, Player nextPlayer,
                         const MiscNNInputParams& /*nnInputParams*/, int nnXLen, int nnYLen, bool useNHWC,
                         float* rowBin, float* rowGlobal)
{
  assert(nnXLen <= NNPos::MAX_BOARD_LEN);
...
void NNInputs::fillRowV5(const Board& board, const BoardHistory& hist, Player nextPlayer,
                         const MiscNNInputParams& /*nnInputParams*/, int nnXLen, int nnYLen, bool useNHWC,
                         float* rowBin, float* rowGlobal)
{
  assert(nnXLen <= NNPos::MAX_BOARD_LEN);
...
void NNInputs::fillRowV6(const Board& board, const BoardHistory& hist, Player nextPlayer,
                         const MiscNNInputParams& /*nnInputParams*/, int nnXLen, int nnYLen, bool useNHWC,
                         float* rowBin, float* rowGlobal)
{
  assert(nnXLen <= NNPos::MAX_BOARD_LEN);
...
void NNInputs::fillRowV7(const Board& board, const BoardHistory& hist, Player nextPlayer,
                         const MiscNNInputParams& /*nnInputParams*/, int nnXLen, int nnYLen, bool useNHWC,
                         float* rowBin, float* rowGlobal)
{
  assert(nnXLen <= NNPos::MAX_BOARD_LEN);
